...
function getRoutes(version, path) {
  ...
  app.use(
    `/api/${version}/agency`,
    agencyRouter.agencyRoutes(new AgencyService())
  );
  ...
  app.use(
    `/api/${version}/taxonomy`,
    taxonomyRouter.taxonomyRoutes(new TaxonomyService())
  );
  ...
  app.use(
    ...
    locationRouter.locationRoutes(new LocationService())
  );
